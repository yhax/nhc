...
class NHCChildTemplateSensor(SensorEntity):
    ...
    def __init__(self, name, unique_id, state_template, attribute_templates):
        ...
        self._attr_name = name
        self._attr_unique_id = unique_id
        self._attr_icon = "mdi:cloud-question" # Base fallback icon
    ...
    @property
    def icon(self) -> str:
        """Dynamically return the icon based on the active storm classification state value."""
        state_str = str(self._attr_native_value).lower()
        
        # If the slot has no active storm assigned
        if "unavailable" in state_str or "unknown" in state_str or "waiting" in state_str:
            return "mdi:cloud-question"
            
        # Parse the storm suffix string generated by your get_state macro
        if ", td" in state_str:
            return "mdi:weather-dust"      # Visual anchor for Tropical Depression
        elif ", ts" in state_str:
            return "mdi:alpha-s-circle-outline"   # Open spiral icon for Tropical Storm
        elif "cat" in state_str:
            return "mdi:weather-hurricane" # Full hurricane symbol wrapper
